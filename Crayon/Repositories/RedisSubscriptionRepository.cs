...
public class RedisSubscriptionRepository(ConnectionMultiplexer redis, ISubscriptionRepository repository) : RedisGenericCache(redis), ISubscriptionRepository
{
    ...
    public async Task UpdateSubscriptionToPurchased(Guid id, DateTime expires, List<Guid> licences, Guid accountId)
    {
        ...
        await RemoveItem($"{nameof(SubscriptionRepository)}_{nameof(GetSubscriptions)}_{accountId}");
        await RemoveItem($"{nameof(SubscriptionRepository)}_{nameof(GetLicences)}_{accountId}");
    }
